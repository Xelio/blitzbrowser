...
import { SESSION_COOKIE_NAME, hashAuthKey, expectedHashedAuthKey, isAuthRequired, isSessionValid } from '../../auth-check.ts';

const is_https_required = env.HTTPS_DISABLED !== 'true';

export function isAuthenticated(cookies: Cookies) {
    return isSessionValid(cookies.get(SESSION_COOKIE_NAME));
}

export async function authenticate(auth_key: string, cookies: Cookies) {
    if (!isAuthRequired() || hashAuthKey(auth_key) !== expectedHashedAuthKey) {
        return false;
    }
